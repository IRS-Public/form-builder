...
// coupling FormativeApp exists to prevent, so the bundle locates *itself* instead.
//
// It can, because the path is a fixed contract rather than a per-app choice: FormativeAssets.scala
// extracts this bundle out of the formative jar into `<resources>/vendor/formative/`, and the
// generator publishes an app's `website-static` as its `resources/` directory. So this module is
// always served from
//
//     {appBase}/resources/vendor/formative/flow-runtime/js/
//
...
//
// `VENDOR_TAIL` below therefore has to match FormativeAssets.vendorPath. A mismatch does not throw:
...
const VENDOR_TAIL = '/vendor/formative/flow-runtime/js/'
