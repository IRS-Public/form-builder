...
def generatedFlowContentPath(app: FormativeApp) = app.localesDir / s"flow_${app.defaultLocale}.yaml"
private def translatedFlowContentPath(app: FormativeApp, languageCode: String) =
...
/** The chrome strings the scaffold's own templates ask for, shipped in this jar.
  ...
  * field's month names, a modal's close button. It is the same text in every Formative app, and it was duplicated
...
def chromeLocaleContent(languageCode: String): Option[Json] =
  ...
  Option(classOf[Locale].getResourceAsStream(s"/formative/locales/$languageCode.yaml")).map { stream =>
    val text =
...
case class Locale(languageCode: String, app: FormativeApp) {
...
/** Generate the flow_en.yaml locale file.
  ...
  */
def generateFlowLocaleFile(translationMap: mutable.LinkedHashMap[String, Any], app: FormativeApp): Unit = {
...
/** Re-sync every non-English `flow_{lang}.yaml` to the current `flow_en.yaml` key set.
  *
  * Intended to be called in-process by the Author Mode save endpoint (package `gov.irs.formative.authoring`)
  ...
  */
def syncTranslationLocales(app: FormativeApp): Unit = {
  val generatedPath = generatedFlowContentPath(app)
...
private def syncTranslationLocale(app: FormativeApp, locale: String, englishContent: Json): Unit = {
