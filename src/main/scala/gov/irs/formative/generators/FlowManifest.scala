...
import gov.irs.formative.parser.{ Flow, Page }
...
/** Emits a JSON manifest used by client-side navigation in single-question-per-screen mode.
  ...
  */
object FlowManifest {
  def buildJson(flow: Flow, languageCode: String, app: FormativeApp): Json =
    Json.fromValues(flow.pages.map(page => pageJson(page, languageCode, app)))

  private def pageJson(page: Page, languageCode: String, app: FormativeApp): Json = {
