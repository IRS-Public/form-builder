...
import gov.irs.formative.Locale
...
case class FormativeMessageResolver(locale: Locale) extends AbstractMessageResolver:
...
/** Renders the scaffold's Thymeleaf templates for one language.
  ...
  * next to the parser it registered in [[FormativeApp.nodeTypes]].
  *
  ...
  * was possible at all.
  */
class FormativeTemplateEngine(languageCode: String, val app: FormativeApp) {
  ...
  private val libraryResolver = resolverFor("/formative/templates/", 2)

  private val locale = Locale(languageCode, app)
  private val templateEngine = new TemplateEngine()
  val messageResolver = FormativeMessageResolver(locale)
