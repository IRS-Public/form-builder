...
/** Emits the **Formative Graph Model** — the JSON Fact Explorer renders as an interactive graph.
  *
  ...
  * via [[FormativeApp.nodeTypes]] has them listed there, and Fact Explorer accepts exactly those and no others — an
  ...
  * wherever it is served, so building with `--formativeGraph` by default would quietly hand Fact Explorer the sparser
...
object FormativeGraph {
  ...
  /** Every `<Fact>` in the merged dictionary, **last definition per path wins**.
    ...
    */
  private def factsFrom(dictionaryXml: Elem, app: FormativeApp): List[FactNode] = {
  ...
  /** Build the graph.
    ...
    *
    * @param flowConfig
    *   the resolved flow XML (`Formative.resolvedFlowConfig`) — the source of every element and its `rawXml`
    * @param dictionaryXml
    *   the merged fact dictionary
    * @param flow
    *   the parsed flow — the source of page routes, titles and module grouping
    */
  def buildJson(flowConfig: Elem, dictionaryXml: Elem, flow: Flow, app: FormativeApp): Json = {
