...
/** Anchor for the resource lookup in [[FormativeAssets]].
  *
  ...
  */
final private class FormativeAssetsAnchor

/** The library's own browser assets, and the one mechanism that gets them out of this jar into a generated site.
  *
  * Formative ships the general front-end interface every app renders with — the theme stylesheet (design tokens, page
  * layout, and the styling of every element the generators emit) and the flow runtime (the custom elements a generated
  * questionnaire actually runs on). Neither has anything to do with which app it is, and neither is optional: without
  * them a generated site has no styling and no working questionnaire.
  *
  ...
  *
  * [[Formative.regenerate]] explains at length why flow, facts and an app's locales are read with `os.read`: Author
  ...
  *
  * The one consequence worth knowing: editing a stylesheet under `theme/styles/` inside `/formative/` *during* a `~run`
  * session hits exactly the staleness that comment describes. Republish (`sbt publishLocal`) and restart the app rather
  * than expecting a live reload.
  */
object FormativeAssets {

  /** Where these assets live on the classpath. */
  val resourceRoot = "/formative/website-static"
  ...
  /** Where they land beneath a generated site's `resources/`.
    ...
    * (`${basePath}/resources/vendor/formative/...`) — Thymeleaf cannot read a Scala constant. Changing this means
    * grepping the templates for `vendor/formative`.
    *
    ...
    * levels up from `vendor/formative/theme/styles/<dir>/` is `vendor/`, exactly as it was from
    * `vendor/taxpert/theme/styles/<dir>/`. Do not flatten the tree.
    */
  val vendorPath: Seq[String] = Seq("vendor", "formative")
  ...
  def extractTo(target: os.Path): Unit = {
    val url = Option(classOf[FormativeAssetsAnchor].getResource(resourceRoot)).getOrElse {
      throw new IllegalStateException(
        s"$resourceRoot is missing from the formative jar. The theme and the flow runtime ship as classpath " +
          "resources; a build that cannot find them would silently generate a site with no CSS.",
      )
    }

    // Both branches are load-bearing. Running inside /formative (`sbt test`) the resources are loose files under
