...
/** The Form Graph Model generator, against Pet Planner.
  ...
  */
class FormBuilderGraphSpec extends AnyFunSpec {
  ...
  private val graph = FormBuilderGraph.buildJson(formConfig, dictionaryConfig, flow, FixtureApp.app)
  ...
  describe("facts") {
    ...
    it("keeps only the last definition of a redefined path") {
      ...
      val g = FormBuilderGraph.buildJson(formConfig, redefined, flow, FixtureApp.app)
  ...
  describe("the flowTags contract") {
    ...
    it("declares an app-registered node type, so the consumer's validate() accepts it") {
      ...
      val custom = FixtureApp.app.copy(nodeTypes = Map("pp-feeding-schedule" -> gov.irs.formbuilder.parser.FgDetail))
      val withCustom = <FlowConfig>
        <page route="/" title="About your pet">
          <pp-feeding-schedule path="/petName" meals-per-day="2"/>
        </page>
      </FlowConfig>
      val g = FormBuilderGraph.buildJson(withCustom, dictionaryConfig, flow, custom)
  ...
  describe("path resolution") {
    it("handles absolute, dotted and parent-relative forms") {
      FormBuilderGraph.resolveDependencyPath("/a/b", "/x/y") shouldBe "/a/b"
      FormBuilderGraph.resolveDependencyPath("../c", "/a/b") shouldBe "/c"
      FormBuilderGraph.resolveDependencyPath("./c", "/a/b") shouldBe "/a/c"
      FormBuilderGraph.resolveDependencyPath("c", "/a/b") shouldBe "/a/c"
    }
  }
}
